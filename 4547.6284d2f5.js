/**
 * =============================================================================
 * FILE: 4547.ee558c8c.js (Webpack Chunk ID: 4547)
 * PURPOSE: Enemy Type Definitions and Wave Progression Data
 * CONTENT:
 *   - Module 1519: Enemy type catalog (bug, bigbug, ant, worm, bear, etc.)
 *   - Module 20984: Enemy effect enum (Small, Hidden, Immune, etc.)
 *   - Module 28325: Wave class builder for game progression
 *   - Module 35060: Enemy wave configuration (spawning rules, timing)
 *   - Module 43236: Wave progression levels 1-10 (10 different difficulty sets)
 *   - Module 58991: Challenge/story mode levels 0-30 (31 waves with modifiers)
 *   - Module 65452: Advanced modifiers and effect management
 * TECHNOLOGY: JavaScript classes, wave builders, enemy data structures
 * SIZE: ~36 KB
 * =============================================================================
 */
...
(self["webpackChunkgame"] = self["webpackChunkgame"] || []).push([
  [4547],
  {
    // ====== MODULE 1519: Enemy Type Definitions ======
    // Catalog of all enemy types with stats, effects, and behavior
    // Exports: X (main enemy types export)
    1519: function (e, d, a) {
      a.d(d, {
        X: function () {
          return o;
      ...
      const o = {
        ...
        bug: {
          ...
          path: { speed: 22 }, // Moderate speed
        },
        // ===== TIER 2: Tougher Enemies =====
        bigbug: {
          key: "bigbug",
          name: "Big Bug",
          description:
            "A heavy duty bug. Vastly stronger than the standard bug, but much slower.",
          options: { health: 10, resistance: [], weakness: [], value: 10 }, // 10 HP, 10 reward gold
          path: { speed: 14 }, // Slower movement
        },
        // ===== SPECIAL MECHANIC: Small/Hard to Hit =====
        ant: {
          key: "ant",
          name: "Ant",
          description:
          ...
          options: {
            ...
            weakness: [],
            value: 1,
            effects: t([n.R.Small, -1]), // Apply "Small" effect permanently (hard to target)
          },
          path: { speed: 25 }, // Very fast to compensate for low health
        },
        // ===== SPECIAL MECHANIC: Underground/Hidden =====
        worm: {
          key: "worm",
          name: "Worm",
          description:
          ...
          options: {
            ...
            weakness: [],
            value: 1,
            effects: t([n.R.Hidden, -1]), // Apply "Hidden" effect (invulnerable until surface)
          },
          path: { speed: 19, lockRotation: !0, preventKnockback: !0 }, // Can't be knocked back while hidden
        },
        // ===== SPECIAL MECHANIC: Spawner Enemy =====
        bugbus: {
          key: "bugbus",
          name: "Bug Bus",
          description:
          ...
          options: {
            ...
            weakness: [],
            value: 10,
            spawnsEnemies: !0, // When killed, spawns other enemies
          },
          path: { speed: 15 },
        },
        // ===== BOSS ENEMY: High Health & Damage =====
        bear: {
          key: "bear",
          name: "Bear",
          description:
            "A bear is looking for honey! Don't let it reach the gate or it will cause massive damage!",
          options: { health: 150, resistance: [], weakness: [], value: 100 }, // 150 HP, 100 reward gold
          path: { speed: 14 },
        },
        // ===== DECORATIVE: Bear Part (from boss death animation) =====
        bear_arm: {
          key: "bear_arm",
          name: "[bear arm piece]",
          options: {
            ...
            weakness: [],
            value: 0,
            effects: t([n.R.Immune, -1]), // Immune to damage (can't be killed)
          },
          path: { speed: 0, progress: 0.1 }, // Doesn't move (static decoration)
        },
      };
    },
    ...
    20984: function (e, d, a) {
      ...
      (function (e) {
        // ===== DEFENSIVE EFFECTS =====
        e[(e["Honey"] = 0)] = "Honey"; // Bee attack effect: slows enemy movement
        e[(e["Poison"] = 1)] = "Poison"; // Bee attack effect: damage over time
        e[(e["Fire"] = 2)] = "Fire"; // Bee attack effect: burning damage
        e[(e["Burn"] = 3)] = "Burn"; // State from Fire effect: continued burning
        e[(e["Ice"] = 4)] = "Ice"; // Bee attack effect: temporary freeze

        // ===== MOVEMENT/SPEED EFFECTS =====
        e[(e["Fast"] = 5)] = "Fast"; // Speed boost - enemy moves faster
        e[(e["Slow"] = 6)] = "Slow"; // Speed debuff - enemy moves slower
        e[(e["Frozen"] = 10)] = "Frozen"; // Complete halt - enemy cannot move
        e[(e["Stopped"] = 14)] = "Stopped"; // Stopped state (similar to frozen)

        // ===== DEFENSIVE/WEAKNESS EFFECTS =====
        e[(e["Weak"] = 7)] = "Weak"; // Takes extra damage from all attacks
        e[(e["Shield"] = 8)] = "Shield"; // Absorbs one hit without damage
        e[(e["Immune"] = 9)] = "Immune"; // Cannot be damaged (boss parts, etc.)

        // ===== SPECIAL ENEMY MECHANICS =====
        e[(e["Small"] = 11)] = "Small"; // Hard to target (ants) - reduces hit chance
        e[(e["Hidden"] = 12)] = "Hidden"; // Underground/invisible - cannot be attacked
        e[(e["Aggro"] = 13)] = "Aggro"; // Increased aggression - prioritizes attacking
        e[(e["Corroding"] = 15)] = "Corroding"; // Slowly breaking down: loses health over time
      })(n || (n = {}));
    },
    ...
    28325: function (e, d, a) {
      ...
      // ====== MODULE 28325: Wave Round Builder ======
      // Builds a "round" (collection of sequential waves) for game progression
      // Used to group multiple waves together for a level/difficulty tier
      class t {
        constructor() {
          (0, n.A)(this, "round", { waves: [] });
        }

        // Add a wave to this round's wave array
        add(e) {
          return (this.round.waves.push(e), this);
        }

        // Finalize and return the complete round object
        done() {
          return this.round;
        }
      }
    ...
    // ====== MODULE 35060: Wave Configuration Class ======
    // Configures a single wave: which enemies spawn, how fast, in what order
    35060: function (e, d, a) {
      ...
      class t {
        // constructor(delay, spawnRate, shuffle)
        // delay: ms to wait before wave starts
        // spawnRate: spawn frequency (1 = normal, 2 = double speed, etc)
        // shuffle: randomize enemy spawn order if true
        ...
        // key: enemy type (bug, bigbug, ant, etc)
        // d: quantity of this enemy to spawn
        // a: rank/tier (affects difficulty/scaling)
        // n: array of effects to apply to enemy
        add(e, d = 1, a = 1, n = []) {
          for (let t = 0; t < d; t++)
        ...
        // pause(duration in waves)
        // Add empty spawn slots to create time gap before next enemy
        pause(e = 1) {
          return this.add("", e);
        }
    ...
    43236: function (e, d, a) {
      ...
      // ====== MODULE 43236: Wave Progression & Generation ======
      // Generates enemy waves based on difficulty level and randomization
      // Scaling functions for health, rewards, and damage as difficulty increases

      // Difficulty scaling formula - scales enemy health/difficulty
      // Roughly 1.5x per difficulty level
      function s(e, d) {
        if (d <= 1) return e;
        if (1 === e && 3 === d) return 3;
        let a = e;
      ...
      // Reward scaling - scales gold/points earned
      // 1.5x per difficulty level
      function i(e, d) {
        return d <= 1 ? e : Math.round(e + 1.5 * d);
      }

      // Simple multiplier scaling - direct multiplication by difficulty
      function r(e, d) {
        if (d <= 1) return e;
        const a = e;
        return a * d;
      ...
      // Get enemies spawned by bugbus (Bug Bus) in a given wave
      // Returns array of enemy definitions that spawn from bugbus
      function g(e, d) {
        if ("bugbus" === e) {
          const e = h[d];
          return [...e.enemies];
      ...
      // Generate a complete round based on wave index and difficulty
      // Different patterns: bear (boss), mixed (random), fixed (consistent), silly, hard
      function u(e, d) {
        const a = b(e, d),
          s = e % 10,
          i = new t.N();
        let r = "mixed";
        if (
          // Select wave pattern based on wave number modulo 10
          // Different patterns trigger different behavior
          ((r =
            9 === s
              ? "bear"
              : s >= 0 && s < 5
          ...
          // Boss wave: single bear with various effect modifiers based on difficulty
          const e = d > 0.3 && d < 0.8,
        ...
        if ("mixed" === r) {
          // Mixed wave: variety of enemy types, shuffled spawn order
        ...
        if ("fixed" === r) {
          // Fixed wave: consistent enemy types, shuffled order
          const e = new n.Q(0, 2, !0),
            t = w(a, d);
          for (let d = 0; d < t.length; d++) {
            const a = t[d];
        ...
        if ("silly" === r) {
          // Silly wave: 100x spawn of single random enemy type (chaos mode)
        ...
        if ("hard" === r) {
          // Hard wave: 3x spawn of random enemy, higher difficulty scaling
          const e = ["bug", "bigbug", "bug", "ant", "worm", "bugbus"],
            t = e[Math.floor(e.length * d)] || "bug";
          return i
            .add(
        ...
        // Default/fallback: 300 basic bugs
        return i.add(new n.Q(0, 10, !1).add("bug", 300, 1).done()).done();
      }

      // Calculate actual difficulty level from wave index and player difficulty multiplier
      function b(e, d) {
